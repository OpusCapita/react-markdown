...
describe('<MarkdownOrderedListButton/>', () => {
  it('check active button', () => {
    const state = Plain.deserialize("1. some text");
    const wrapper = shallow(<MarkdownOrderedListButton state={state} onChange={() => {}} />);
  ...
  it('check unwrap header', () => {
    ...
    const wrapper = shallow(<MarkdownOrderedListButton state={state} onChange={(newState) => { state = newState }} />);
