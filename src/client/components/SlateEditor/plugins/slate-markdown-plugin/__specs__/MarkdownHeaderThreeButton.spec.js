...
describe('<MarkdownHeaderThreeButton/>', () => {
  it('check active button', () => {
    const state = Plain.deserialize("### some text");
    const wrapper = shallow(<MarkdownHeaderThreeButton state={state} onChange={() => {}} />);
  ...
  it('check unwrap header', () => {
    ...
    const wrapper = shallow(<MarkdownHeaderThreeButton state={state} onChange={(newState) => { state = newState }} />);
