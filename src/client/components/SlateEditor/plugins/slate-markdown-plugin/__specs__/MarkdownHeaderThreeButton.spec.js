...
describe('<MarkdownHeaderOneButton/>', () => {
  it('check active button', () => {
    const state = Plain.deserialize("# some text");
    const wrapper = shallow(<MarkdownHeaderOneButton state={state} onChange={() => {}} />);
  ...
  it('check unwrap header', () => {
    ...
    const wrapper = shallow(<MarkdownHeaderOneButton state={state} onChange={(newState) => { state = newState }} />);
