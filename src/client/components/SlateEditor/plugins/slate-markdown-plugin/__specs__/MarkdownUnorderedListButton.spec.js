...
describe('<MarkdownUnorderedListButton/>', () => {
  it('check active button', () => {
    const state = Plain.deserialize("* some text");
    const wrapper = shallow(<MarkdownUnorderedListButton state={state} onChange={() => {}} />);
  ...
  it('check unwrap header', () => {
    ...
    const wrapper = shallow(<MarkdownUnorderedListButton state={state}
