...
describe('<MarkdownHeaderSixButton/>', () => {
  it('check active button', () => {
    const state = Plain.deserialize("###### some text");
    const wrapper = shallow(<MarkdownHeaderSixButton state={state} onChange={() => {}} />);
  ...
  it('check unwrap header', () => {
    ...
    const wrapper = shallow(<MarkdownHeaderSixButton state={state} onChange={(newState) => { state = newState }} />);
