...
describe('<MarkdownHeaderFiveButton/>', () => {
  it('check active button', () => {
    const state = Plain.deserialize("##### some text");
    const wrapper = shallow(<MarkdownHeaderFiveButton state={state} onChange={() => {}} />);
  ...
  it('check unwrap header', () => {
    ...
    const wrapper = shallow(<MarkdownHeaderFiveButton state={state} onChange={(newState) => { state = newState }} />);
