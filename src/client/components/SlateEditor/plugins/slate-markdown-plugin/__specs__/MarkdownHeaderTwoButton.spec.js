...
describe('<MarkdownHeaderTwoButton/>', () => {
  it('check active button', () => {
    const state = Plain.deserialize("## some text");
    const wrapper = shallow(<MarkdownHeaderTwoButton state={state} onChange={() => {}} />);
  ...
  it('check unwrap header', () => {
    ...
    const wrapper = shallow(<MarkdownHeaderTwoButton state={state} onChange={(newState) => { state = newState }} />);
